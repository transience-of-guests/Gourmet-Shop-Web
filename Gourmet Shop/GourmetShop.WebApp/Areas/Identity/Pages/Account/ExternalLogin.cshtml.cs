...
namespace GourmetShop.WebApp.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        _logger.LogInformation("UserInfo created an account using {Name} provider.", info.LoginProvider);
        ...
        private Authentication CreateUser()
        {
            try
            {
                return Activator.CreateInstance<Authentication>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(Authentication)}'. " +
                    $"Ensure that '{nameof(Authentication)}' is not an abstract class and has a parameterless constructor, or alternatively " +
